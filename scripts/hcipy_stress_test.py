...
DATA_DIR = os.path.join(BASE, 'data', 'hcipy_dataset')
...
def main():
    ...
    sample_img_path = os.path.join(DATA_DIR, 'frame_0000.npy')
    ...
    for _ in range(1000):
        ...
        lib.compute_slopes_enhanced(img_ptr, slopes_ptr, mask_ptr, n_sub, n_valid, sub_px, 0.0, 0.0, 0.0)
    ...
    print(f"Total memory growth after 1000 iterations: {mem_growth:+.3f} MB")
    ...
    zero_frame = np.zeros(sample_img.size, dtype=np.float32)
    lib.compute_slopes_enhanced(
        zero_frame.ctypes.data_as(ct.POINTER(ct.c_float)),
        slopes_ptr, mask_ptr, n_sub, n_valid, sub_px, 0.0, 0.0, 0.0
    )
    is_nan_zero = np.isnan(c_slopes).sum()
    is_inf_zero = np.isinf(c_slopes).sum()
    print(f"Zero intensity frame: NaN count = {is_nan_zero} | Inf count = {is_inf_zero} | Centroid values: {c_slopes[0]:.2f} (expected: (sub_px-1.0)*0.5 = { (sub_px - 1.0) * 0.5 })")
    ...
    saturated_frame = np.ones(sample_img.size, dtype=np.float32) * 255.0
    lib.compute_slopes_enhanced(
    ...
    lib.compute_slopes_enhanced(
        ...
        n_sub, n_valid - 10, sub_px, 0.0, 0.0, 0.0
    )
    ...
    lib.compute_slopes_enhanced(
        ...
        slopes_ptr, mask_ptr, n_sub, n_valid, sub_px,
        ct.c_float(0.0), ct.c_float(0.0), ct.c_float(0.0)
    )
