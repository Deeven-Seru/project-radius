"""
Fit and apply per-subaperture multiplicative gains to C-engine slopes.
This estimates, for each valid subaperture and for each axis, a scalar gain s that
minimizes ||truth - s * pred|| over available frames (zero intercept LS).
"""
import os
import ctypes as ct
import numpy as np
...
def main():
    ...
    # Use best params found earlier: bg_multiplier=2.0, shifts=0
    bg_multiplier = 2.0
    shift_x = 0.0
    shift_y = 0.0
    bg_base = 0.0
    ...
    for fi in range(n_frames):
        ...
        if lib.has_enhanced:
            lib.compute_slopes_enhanced(
                ...
                ct.c_int(n_valid),
                ct.c_int(sub_px),
                ct.c_float(bg_base * bg_multiplier),
                ct.c_float(shift_x),
                ct.c_float(shift_y),
            )
        else:
    ...
    # Fit multiplicative gains per subap and axis (zero intercept)
    gains = np.zeros(2*n_valid, dtype=np.float32)
    eps = 1e-8
    for j in range(2*n_valid):
        p = preds[:, j]
        t = truths[:, j]
        denom = np.sum(p * p) + eps
        gains[j] = np.sum(p * t) / denom

    preds_scaled = preds * gains[np.newaxis, :]

    # Evaluate
    frame_metrics = [compute_metrics(preds_scaled[i], truths[i]) for i in range(n_frames)]
    frame_metrics = np.array(frame_metrics, dtype=np.float32)
    avg = np.mean(frame_metrics, axis=0)

    print('Per-subap gains applied (first 10 x,y pairs):')
    for i in range(min(10, n_valid)):
        print(f'  subap {i:03d}: gx={gains[i]:.4f} gy={gains[i+n_valid]:.4f}')

    print('\nAfter gains: Average MSE: {:.4e}  RMSE: {:.4e}  R2(avg): {:.4f}'.format(avg[0], avg[1], avg[2]))
